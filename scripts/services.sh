...
#check from all users account have passowrd
 ...
 if [[ $service == "user_password" ]] && [[ ! -z $password_current_val ]]s
    then echo "user-setting[$SET]:($message ,but {$password_current_val } user dont have one lock it until implement a password). >> "$MESSAGES_FILE"
...
#check the UID value for root user
  ...
  then
    echo "user-setting[$SET]:($message , the current acount is {$UID_current_val} but the recommanded value is {$recom_val1}). >> "$MESSAGES_FILE"
 fi
#//////////////////////////////
#check if there any netrc files
 ...
 if [[ $service == "FTP_service" ]] && [[ $netrc_current_val -ne null ]]
   then  echo "user-setting[$SET]:($message, the $service return with files $netrc_current_val {not null}  but the recommended value is {$recom_val1} or {$recom_val2}) >> "$MESSAGES_FILE"
...
#check from forward files
 ...
 if [[ $service == "forward_file" ]] && [[ $forward_current_val -ne null ]]
  then  echo "user-setting[$SET]:($message ,the $service return with files $forward_current_val {not null} but the recommanded value is {$recom_val1} or {$recom_val2}). >> "$MESSAGES_FILE"
        echo "///////////////////////////////"
...
#check from rhost files
 ...
 if [[ $service == "rhost_file" ]] && [[ $rhost_current_val -ne null ]]
    then    echo "user-setting[$SET]:($message,the $service return with files $rhost_current_val {not null} but the recommanded value is {$recom_val1} or {$recom_val2}). >> "$MESSAGES_FILE"
            echo "///////////////////////////////"
        #echo "to remove these return files use the following file"
        #./file
...
#check if there duplicate in users ID
 ...
 if [[ $service == "duplicate_uid" ]] && [[ $Duplicate_UID -ne null ]]
    then echo "user-setting[$SET]:($message, there are these duplicated user IDs $Duplicate_UID {not null} but it recommanded to be {$recom_val1} or {$recom_val2}). >> "$MESSAGES_FILE"
...
#check if there duplicate in group ID
 ...
 if [[ $service == "duplicate_gid" ]] && [[ $Duplicate_GID -ne null ]]
    then echo "user-setting[$SET]:($message, there are these duplicated group IDs $Duplicate_GID {not null} but it recommanded to be {$recom_val1} or {$recom_val2}). >> "$MESSAGES_FILE"
...
if [[ $service == "duplicated_username" ]] && [[ ! -z $username_currentval ]]
    then echo "user-setting[$SET]:($message,but there a duplicate in it which is not recommended). >> "$MESSAGES_FILE"
...
if [[ $service == "duplicated_groupname" ]] && [[ ! -z $groupname_currentval ]]
    then echo "user-setting[$SET]:($message,but there a duplicate in group name it which is not recommended). >> "$MESSAGES_FILE"
...
if [[ $service == "group_passwd_file" ]] && [[ ! -z $exist_file ]]
    then echo "user-setting[$SET]:($message,but there this GID in /etc/passwd but does not exist in /etc/group). >> "$MESSAGES_FILE"
...
if [[ $service == "dir_file_unowned" ]] && [[ ! -z $dir_currentval ]] 
    then
  echo "user-setting[$SET]:($message, but there these {$dir_currentval} file or directory are unowned check them). >> "$MESSAGES_FILE"
...
#check there is no "+" in passwd file 
 ...
 if [[ $service == "legacy_passwd" ]] && [[ $passwd_current_val -ne null ]]
    then echo "user-setting[$SET]:($message,its not recommanded to be a "'+'" in passwd file). >> "$MESSAGES_FILE"   
...
#check there is no "+" in group file
 ...
 if [[ $service == "legacy_group" ]] && [[ $group_current_val -ne null ]]
    then echo "user-setting[$SET]:($message,its not recommanded to be a "'+'" in group file). >> "$MESSAGES_FILE"  
...
#check there is no "+" in shadow file
 ...
 if [[ $service == "legacy_shadow" ]] && [[ $shadow_current_val -ne null ]]
    then echo "user-setting[$SET]:($message,its not recommanded to be a "'+'" in shadow file). >> "$MESSAGES_FILE"    
...
if [[ $service == "su_restricted" ]] && [[ ! -z $su_currentval ]]
  then echo "user-setting[$SET]:($message). >> "$MESSAGES_FILE"
