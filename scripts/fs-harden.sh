...
check-mount-options()	{
	...
	for opt in "${REC_MOUNT_OPTIONS[@]}"; do
		...
		echo "FileSystem-Hardening[mounts]($L_MOUNT_POINT) $opt 0" >> $STATUS_FILE
...
cmp-fstab()	{
	...
	if [[ $FSTAB_DEVICE == $L_DEVICE ]] then
		echo "FileSystem-Hardening[fstab]($L_MOUNT_POINT) $L_DEVICE 0" >> $STATUS_FILE
	...
	if [[ $FSTAB_FS_TYPE == $L_FS_TYPE ]] then
		echo "FileSystem-Hardening[fstab]($L_MOUNT_POINT) $L_FS_TYPE 0" >> $STATUS_FILE
	...
	for opt in $FSTAB_MOUNT_OPTIONS; do
		[[ $L_MOUNT_OPTIONS =~ (^|[[:space:]])"$opt"($|[[:space:]]) ]] && continue
		echo "FileSystem-Hardening[fstab]($L_MOUNT_POINT) $opt 0" >> $STATUS_FILE
...
check-mount-point()	{
	...
	if [[ -n ${MOUNT_POINTS[$L_MOUNT_POINT]} ]] then
		...
		if [[ $L_FS_TYPE != $REC_FS_TYPE ]] then
			echo "FileSystem-Hardening[mounts]($L_MOUNT_POINT) $L_FS_TYPE 0" >> $STATUS_FILE
