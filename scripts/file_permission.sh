...
#check the status for passwd file and compare it with recommanded    
 ...
 if [[ $service == "passwd_file" ]] && [[ -z $permission_passwd ]]
    then echo "file-permission[$PERM]:(the current permission nad owners for passwd file not as recommended). $message" >> "$MESSAGES_FILE"
    echo "///////////////////////////////////////"
...
permission_group=$(find /etc/group -perm 644 -user root -group root)
 if [[ $service == "group_file" ]] && [[ -z $permission_group ]]
    then echo "file-permission[$PERM]:(the current permission or owners for group file not as recommended). $message" >> "$MESSAGES_FILE"
    echo "///////////////////////////////////////"
...
permission_shadow=$(find /etc/shadow -perm 600 -user root -group root)
 if [[ $service == "shadow_file" ]] && [[ -z $permission_shadow ]]
    then echo "file-permission[$PERM]:(the current permission or owners for shadow file not as recommended). $message" >> "$MESSAGES_FILE"
    echo "///////////////////////////////////////"
...
permission_gshadow=$(find /etc/gshadow -perm 600 -user root -group root)
 if [[ $service == "gshadow_file" ]] && [[ -z $permission_gshadow ]]
    then echo "file-permission[$PERM]:(the current permission or owners for gshadow file not as recommended). $message" >> "$MESSAGES_FILE"
    echo "///////////////////////////////////////"
...
permission_passwdb=$(find /etc/passwd- -perm 600 -user root -group root)
 if [[ $service == "passwdb_file" ]] && [[ -z $permission_passwdb ]]
    then echo "file-permission[$PERM]:(the current permission or owners for passwd- file not as recommended). $message" >> "$MESSAGES_FILE"
    echo "///////////////////////////////////////"
...
permission_bootgrub=$(find /boot/grub2/grub.cfg -perm 600 -user root -group root)
 if [[ $service == "bootgrub_file" ]] && [[ -z $permission_bootgrub ]]
    then echo "file-permission[$PERM]:(the current permission or owners for grub.cfg file not as recommended). $message ">> "$MESSAGES_FILE"
    echo "///////////////////////////////////////"
...
permission_defaultgrub=$(find /etc/default/grub -perm 644 -user root -group root)
 if [[ $service == "etcgrdefaultgrub_fileub_file" ]] && [[ -z $permission_etcgrub ]]
    then echo "file-permission[$PERM]:(the current permission or owners for grub file not as recommended). $message ">> "$MESSAGES_FILE"
    echo "///////////////////////////////////////"
...
permission_anacrontab=$(find /etc/anacrontab -perm 600 -user root -group root)
 if [[ $service == "anacrontab_file" ]] && [[ -z $permission_anacrontab ]]
    then echo "file-permission[$PERM]:(the current permission or owners for anacrontab file not as recommended). $message ">> "$MESSAGES_FILE"
    echo "///////////////////////////////////////"
...
permission_crontabd=$(find /etc/cron.d/ -type d -perm 600 -user root -group root)
 if [[ $service == "cron.d_dir" ]] && [[ -z $permission_crontabd ]]
    then echo "file-permission[$PERM]:(the current permission or owners for cron.d directory not as recommended). $message" >> "$MESSAGES_FILE"
    echo "///////////////////////////////////////"
...
permission_crondaily=$(find /etc/cron.daily/ -type d -perm 600 -user root -group root)
 if [[ $service == "cron.daily_dir" ]] && [[ -z $permission_crondaily ]]
    then echo "file-permission[$PERM]:(the current permission or owners for cron.daily directory not as recommended). $message" >> "$MESSAGES_FILE"
    echo "///////////////////////////////////////"
...
permission_cronweekly=$(find /etc/cron.weekly/ -type d -perm 600 -user root -group root)
 if [[ $service == "cron.weekly_dir" ]] && [[ -z $permission_cronweekly ]]
    then echo "file-permission[$PERM]:(the current permission or owners for cron.weekly directory not as recommended). $message" >> "$MESSAGES_FILE"
    echo "///////////////////////////////////////"
...
permission_cronmonthly=$(find /etc/cron.monthly/ -type d -perm 600 -user root -group root)
 if [[ $service == "cron.monthly_dir" ]] && [[ -z $permission_cronmonthly ]]
    then echo "file-permission[$PERM]:(the current permission or owners for cron.monthly directory not as recommended). $message" >> "$MESSAGES_FILE"
    echo "///////////////////////////////////////"
...
permission_hostconf=$(find /etc/host.conf -perm 644 -user root -group root)
 if [[ $service == "host.conf_file" ]] && [[ -z $permission_hostconf ]]
    then echo "file-permission[$PERM]:(the current permission or owners for host.conf file not as recommended). $message" >> "$MESSAGES_FILE"
    echo "///////////////////////////////////////"
...
permission_hosts=$(find /etc/hosts -perm 644 -user root -group root)
 if [[ $service == "hosts_file" ]] && [[ -z $permission_hosts ]]
    then echo "file-permission[$PERM]:(the current permission or owners for hosts file not as recommended). $message" >> "$MESSAGES_FILE"
    echo "///////////////////////////////////////"
...
permission_sshd_config=$(find /etc/ssh/sshd_config -perm 600 -user root -group root)
 if [[ $service == "sshd_config_file" ]] && [[ -z $permission_sshd_config ]]
    then echo "file-permission[$PERM]:(the current permission or owners for sshd_config file not as recommended). $message" >> "$MESSAGES_FILE"
...
permission_resolved=$(find /etc/exports -perm 644 -user root -group root)
 if [[ $service == "exports_file" ]] && [[ -z $permission_resolved ]]
    then echo "file-permission[$PERM]:(the current permission or owners for exports file not as recommended). $message" >> "$MESSAGES_FILE"
...
permission_sudoers=$(find /etc/sudoers -perm 440 -user root -group root)
 if [[ $service == "sudoers_file" ]] && [[ -z $permission_sudoers ]]
    then echo "file-permission[$PERM]:(the current permission or owners for sudoers file not as recommended). $message" >> "$MESSAGES_FILE"
...
permission_spoolcron=$(find /var/spool/cron -type d -perm 600 -user root -group root)
 if [[ $service == "cronspool_file" ]] && [[ -z $permission_spoolcron ]]
    then echo "file-permission[$PERM]:(the current permission or owners for /var/spool/cron file not as recommended). $message" >> "$MESSAGES_FILE"
...
permission_runutmp=$(find /var/run/utmp -perm 664 -user root -group utmp)
 if [[ $service == "varrun_utmp_file" ]] && [[ -z $permission_runutmp ]]
    then echo "file-permission[$PERM]:(the current permission or owners for /var/run/utmp file not as recommended). $message" >> "$MESSAGES_FILE"
...
permission_runutmp=$(find /var/log/wtmp -perm 664 -user root -group utmp)
 if [[ $service == "varlog_wtmp_file" ]] && [[ -z $permission_runutmp ]]
    then echo "file-permission[$PERM]:(the current permission or owners for /var/log/wtmp file not as recommended). $message" >> "$MESSAGES_FILE"
