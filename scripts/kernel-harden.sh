...
module-blacklist()	{
	source "$MAIN_DIR/resources/kernel-blocked-modules.rc"
	...
	for MODULE in "${!MODULE_BLACKLIST[@]}"; do
		...
		[[ $(check-pf module "${MODULE_BLACKLIST}" action) == 1 ]] && echo "echo \"blacklist $MODULE\" >> $MODULE_BLACKLIST_FILE" >> $ACTIONS_FILE
	done
}
